...
void *rcv (void *dummyPt)
{
    ...
    {
    count = read(client_sock, data, sizeof(data)-1);
    if(count == -1)
    {
       string errorMessage = "message not sent";
            char* temp;
            strcpy(temp,errorMessage.c_str());
            perror(temp);
            exit(1);
    }
    cout<<data<<"\n";
    ...
    while(1) 
    {
        ...
        read_size=recv(client_sock, msg, 300,0);
        //cout<<msg;
        //string rcvmsg (msg);       
        cout <<  "\rOther:"  << msg << endl;
        cout << "Me:";
        fflush(stdout);
...
void *snd (void *dummyPt)
{
   /* int count;
    ...
    while(!strcmp(data,"exit") == 0 && data != NULL)
    {
        string str;
        cout<<"\n";
        getline(cin,str);
        strcpy(data, str.c_str());
        count = send(client_sock,data,sizeof(data)-1,0);
        if(count == -1)
        {
            string errorMessage = "message not sent";
            char* temp;
